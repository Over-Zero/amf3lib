...
namespace {
    ...
    TEST (ExceptionsInFixtureAreReportedAsHappeningInTheFixture)
    {
        ...
        CHECK(Strings::StrStr(reporter.lastFailedMessage, UNITTEST_TEXT("xception")));
        CHECK(Strings::StrStr(reporter.lastFailedMessage, UNITTEST_TEXT("fixture")));
        CHECK(Strings::StrStr(reporter.lastFailedMessage, UNITTEST_TEXT("ThrowingThingie")));
    }
    ...
    TEST(TestAddedWithTEST_EXMacroGetsDefaultSuite)
    {
        ...
        CHECK_EQUAL(UNITTEST_TEXT("MacroTestHelper1"),
            macroTestList1.GetHead()->m_details.testName);
        CHECK_EQUAL(UNITTEST_TEXT("DefaultSuite"),
            macroTestList1.GetHead()->m_details.suiteName);
    }
    ...
    TEST(TestAddedWithTEST_FIXTURE_EXMacroGetsDefaultSuite)
    {
        ...
        CHECK_EQUAL(UNITTEST_TEXT("MacroTestHelper2"),
            macroTestList2.GetHead()->m_details.testName);
        CHECK_EQUAL(UNITTEST_TEXT("DefaultSuite"),
            macroTestList2.GetHead()->m_details.suiteName);
    }
    ...
    TEST(FixturesWithThrowingCtorsAreFailures)
    {
        ...
        CHECK(Strings::StrStr(reporter.lastFailedMessage,
            UNITTEST_TEXT("while constructing fixture")));
    }
    ...
    TEST(FixturesWithThrowingDtorsAreFailures)
    {
        ...
        CHECK_EQUAL(1, failureCount);
        CHECK(Strings::StrStr(reporter.lastFailedMessage,
            UNITTEST_TEXT("while destroying fixture")));
    }
    ...
    struct FixtureCtorAsserts
    {
        FixtureCtorAsserts()
        {
            UnitTest::ReportAssert(UNITTEST_TEXT("assert failure"),
                UNITTEST_TEXT("file"), FailingLine);
        }
    };
...
#define INNER_STRINGIFY(X) UNITTEST_TEXT(#X)
