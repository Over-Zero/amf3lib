...
TEST_FIXTURE(XmlTestReporterFixture, MultipleCharactersAreEscaped)
{
    TestDetails const details(
        ...
        UNITTEST_TEXT("suite"),
        UNITTEST_TEXT("filename.h"),
        4321);

    reporter.ReportTestStart(details);
    reporter.ReportFailure(details, UNITTEST_TEXT("\"\"\'\'&&<<>>"));
    ...
    CHECK_EQUAL(Strings::Make(expected), Strings::Make(output.str().c_str()));
}

TEST_FIXTURE(XmlTestReporterFixture, OutputIsCachedUntilReportSummaryIsCalled)
{
    TestDetails const details(UNITTEST_TEXT(""), UNITTEST_TEXT(""), UNITTEST_TEXT(""), 0);

    reporter.ReportTestStart(details);
    reporter.ReportFailure(details, UNITTEST_TEXT("message"));
...
{
    TestDetails const details(
        UNITTEST_TEXT("A Test"),
        UNITTEST_TEXT("suite"),
        UNITTEST_TEXT("A File"),
        4321);

    reporter.ReportTestStart(details);
    reporter.ReportFailure(details, UNITTEST_TEXT("A Failure"));
...
{
    TestDetails const failedDetails(
        UNITTEST_TEXT("FailedTest"),
        UNITTEST_TEXT("suite"),
        UNITTEST_TEXT("fail.h"),
        1);
    reporter.ReportTestStart(failedDetails);
    reporter.ReportFailure(failedDetails, UNITTEST_TEXT("expected 1 but was 2"));
    reporter.ReportTestFinish(failedDetails, 0.1f);

    TestDetails const succeededDetails(
        UNITTEST_TEXT("SucceededTest"),
        UNITTEST_TEXT("suite"),
        UNITTEST_TEXT(""),
        0);
...
{
    TestDetails const failedDetails1(
        UNITTEST_TEXT("FailedTest"),
        UNITTEST_TEXT("suite"),
        UNITTEST_TEXT("fail.h"),
        1);
    TestDetails const failedDetails2(
        UNITTEST_TEXT("FailedTest"),
        UNITTEST_TEXT("suite"),
        UNITTEST_TEXT("fail.h"),
        31);
    ...
    reporter.ReportFailure(failedDetails1, UNITTEST_TEXT("expected 1 but was 2"));
    reporter.ReportFailure(failedDetails2, UNITTEST_TEXT("expected one but was two"));
