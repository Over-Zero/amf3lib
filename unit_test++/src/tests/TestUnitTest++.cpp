...
{
    CHECK_ASSERT(UnitTest::ReportAssert(UNITTEST_TEXT("desc"), UNITTEST_TEXT("file"), 0));
}
...
TEST(CheckThrowMacroFailsOnMissingException)
{
    ...
    {
    public:
        NoThrowTest() : Test(UNITTEST_TEXT("nothrow")) {}
...
TEST(CheckThrowMacroFailsOnWrongException)
{
    ...
    {
    public:
        WrongThrowTest() : Test(UNITTEST_TEXT("wrongthrow")) {}
        virtual void RunImpl() const
        {
