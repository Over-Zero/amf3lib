...
{
    os << "<test"
        << " suite=\"" << Strings::ToMultibyte(result.suiteName) << "\"" 
        << " name=\"" << Strings::ToMultibyte(result.testName) << "\""
...
void XmlTestReporter::AddFailure(std::ostream& os, DeferredTestResult const& result)
{
    ...
    for (DeferredTestResult::FailureVec::const_iterator it = result.failures.begin(); 
         ...
         ++it)
    {
        string const escapedMessage = XmlEscape(Strings::ToMultibyte(it->second));
        string const message = BuildFailureMessage(
            Strings::ToMultibyte(result.failureFile),
