...
namespace UnitTest {
    ...
    {
        return (suiteName == NULL) || !Strings::StrCmp(curTest->m_details.suiteName, suiteName);
    }
    ...
    void TestRunner::RunTest(TestResults* const result, Test* const curTest, int const maxTestTimeInMs) const
    {
        ...
        if (maxTestTimeInMs > 0 && testTimeInMs > maxTestTimeInMs && !curTest->m_timeConstraintExempt)
        {
            ...
            stream << UNITTEST_TEXT("Global time constraint failed. Expected under ")
                << maxTestTimeInMs << UNITTEST_TEXT("ms but took ") << testTimeInMs
                << UNITTEST_TEXT("ms.");
